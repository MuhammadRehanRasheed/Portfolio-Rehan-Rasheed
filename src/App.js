...
function BodyText({ parentHeading = '', head, body, id }) {
  const formatText = (text) => {
    // Check for links (starting with "Live Demo:")
    if (text.includes('Live Demo:')) {
      const [label, url] = text.split(': ');
      // Clean the URL by removing @ and any quotes
      const cleanUrl = url.replace('@', '').replace(/"/g, '').trim();
      return (
    ...
    // Check if text contains a colon (for subheadings)
    if (text.includes(':')) {
      ...
      <footer className='container-fluid p-5 text-white mt-5 bg-black'>
        <div className="container text-center">
          ...
          <div className="social-links mt-4">
            ...
            <a 
              href="https://github.com/MuhammadRehanRasheed" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="social-link"
            >
